...
| | 特性 | 说明 |
|---|---|---|
| 🎯 | **树形层级图** | 以主 agent（实心圆）为根、子代理（空心圆）为分支的层级森林；多主会话并排显示；**四色状态语义**：🟢绿=working（在跑）/ 🔵蓝=完成（未查看）/ ⚪灰=空闲（完成且已双击打开过，已读弱化）/ 🔴红=停滞 |
...
| 🖱️ | **单击折叠，双击直达** | 有子节点的根单击展开/折叠子树（行首 ▸/▾ 同效），双击任意节点打开会话；**双击打开 = 已读**：完成节点由蓝变灰（空闲），跨刷新持久（localStorage）；当前会话带「当前」标记 |
