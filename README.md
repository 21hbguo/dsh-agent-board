...
| 🪟 | **双形态看板** | **悬浮窗**（右上角可拖拽、折叠、隐藏，位置持久化）+ **停靠右侧面板**（AionUi 文件插件风格：右侧常驻一列、内容左移不遮挡，拖拽把手调宽 220–520px、双击复位，标题点击折叠为 0 宽、右侧展开按钮） |
| 🔀 | **形态切换/并存** | ▦ 菜单（悬浮窗 / 停靠右侧面板 / 两者并存 / 隐藏）随时切换；**设置 → 常规 → Agent 看板**下拉同效；侧边栏 foot「Agent 看板」按钮 = 显示/隐藏总开关；全部状态持久化 |
